...
> **#164 现行补充（2026-07-13）**：iLink 使用显式文字协议展示和回答 HumanRequest。请求引用是完整的 128-bit correlation UUID，显示为 `HR-<32 位大写十六进制>`；回答格式为 `/answer <完整引用> <回答>`。任何以 `/answer` 开头的消息，无论格式正确、过期、无权还是引用不存在，都属于协议流量，必须在进入 Agent Turn 前消费，不能交给模型。runtime 通过 correlation 找到唯一请求，再复用 #163 的 exact responder/entrypoint/chat/expiry/idempotency 校验和 resume 状态机。Agent 与 Flow 只能选择 `current_sender`（默认）或 `owner`；实际 typed identity 和路由由 runtime 绑定。iLink 对当前会话使用 `SendText`，对 owner 私聊使用 typed `Push`，并把真实 SDK message id 写进 `DeliveryState`。owner 文字回答只接受绑定 owner 自己的 direct chat；群内 `/answer` 会被消费但拒绝。owner 请求永不回退到原群公开展示；已成功显式投递的请求也不会被 progress renderer 重复发送。发送失败会把 request 原子终结为 `failed` + `delivery_failed`，让 Agent run 或 Flow step 明确失败，不再静默进入 `waiting_human`。成功回答在 ACK 前先完成 inbound dedupe，ACK 超时不会产生第二条确认。Feishu 原生卡片和 WebSocket structured frame 分别留给 #165、#166。
